...
const BurgerIngredients = (props) => {
  ...
  useEffect(() => {
    tabContentRef.current &&
      tabContentRef.current.dispatchEvent(new Event("scroll"));
  }, []);
  ...
  return (
    <div className={styles.wrapper}>
      <span
        className={`${styles.heading} text text_type_main-large mt-10 mb-5`}
      >
        Соберите бургер
      </span>
      ...
      <div className={styles.tabContent} ref={tabContentRef}>
        ...
        <div ref={mainGroupRef}>
          <IngredientsGroup title="Начинки" ingredients={mainIngredients} />
        </div>
      </div>
    </div>
  );
