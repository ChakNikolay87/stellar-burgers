...
const DropTarget: FC<IDropTargetProps> = ({
  id = 0,
  type = "ingredient",
  children,
}) => {
  ...
  const [dropLocation, setDropLocation] = useState<"after" | "before" | null>(
    null
  );
  ...
  const [{ onHover }, dropTarget] = useDrop({
    ...
    drop(
      ...
      monitor: DropTargetMonitor<TIngredientWithID, void>
    ) {
      if (ingredient?.id === id) {
        return;
      }
      ...
      const wrapperYCenter =
        wrapperRect.top + (wrapperRect.bottom - wrapperRect.top) / 2;
    ...
    <div className={styles.wrapper} ref={wrapperRef}>
      ...
      <div
        ...
        className={`${styles.innerWrapper} ${
          type === "bun" ? styles.innerWrapperBun : ""
        }`}
      >
        {children}
      </div>
