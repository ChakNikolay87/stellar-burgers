...
const FeedOrderDetails: FC<TFeedOrderDetailsProps> = ({ order }) => {
  ...
  if (order.status && order.status === OrderStatus.PENDING) {
    status = (
      <div className="text text_type_main-small mt-3 mb-15">Готовится</div>
    );
    ...
    status = (
      <div
        className="text text_type_main-small mt-3 mb-15"
        style={{ color: "#00CCCC" }}
      >
        Выполнен
      </div>
  ...
  let orderData: { ingredientList: ReactNode[]; totalPrice: number } =
    useMemo(() => {
